    ...
    downloadExcelWorkbook('mau_nhap_yeu_cau_ho_tro', [
      {
        ...
        rows: [
          [
            ...
            customerOptions[1]?.label || 'KH001 - Ngân hàng Vietcombank',
            groupOptions[1]?.label || 'HIS L2',
            employeeFormOptions[1]?.label || 'VNPT000001 - System Admin',
            projectOptions[1]?.label || 'DA001 - Dự án VNPT HIS - Vietcombank',
            productOptions[1]?.label || 'SOC_MONITOR - Dịch vụ giám sát SOC',
            'Nguyễn Văn A',
            'HIGH',
            'OPEN',
            today,
      ...
      {
        name: 'KhachHang',
        headers: ['ID', 'Mã KH', 'Tên KH'],
        rows: (customers || []).map((customer) => [customer.id, customer.customer_code, customer.customer_name]),
      },
      {
        name: 'NhanSu',
        headers: ['ID', 'Mã NV', 'Họ tên'],
        rows: (employees || []).map((employee) => [
          employee.id,
          employee.employee_code || employee.user_code || employee.username,
          employee.full_name,
        ]),
      },
    ]);
