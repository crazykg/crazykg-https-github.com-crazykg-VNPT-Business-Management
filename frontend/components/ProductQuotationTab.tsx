...
export const ProductQuotationTab: React.FC<ProductQuotationTabProps> = ({
    ...
    if (activeProductPackages.length > 0) {
      const packageCatalogItems = activeProductPackages.map((productPackage) => {
        ...
        return {
          ...
          productPackageId: String(productPackage.id || ''),
          productCode,
          packageCode,
          label,
          productName,
  ...
  const resolveDraftRowCatalogValue = useCallback(
    (item: ProductQuotationDraft['items'][number]): string => {
      const productPackageId = item.product_package_id ? String(item.product_package_id) : '';
      if (productPackageId !== '') {
        const directPackageValue = `package:${productPackageId}`;
        if (quotationCatalogByValue.has(directPackageValue)) {
          return directPackageValue;
        }
      }
    ...
    [quotationCatalogByValue, quotationCatalogValueByLookupKey, quotationCatalogValuesByProductId]
  );
  ...
  const currentPayload = useMemo<ProductQuotationDraftPayload>(
    () => ({
      ...
      items: normalizedRows.map((row) => ({
        ...
        product_package_id: row.productPackageId ? Number(row.productPackageId) : null,
    ...
    const nextRows = draft.items.length > 0
      ? [...draft.items]
          ...
          .map((item) => ({
            ...
            productPackageId: item.product_package_id ? String(item.product_package_id) : null,
  ...
  const handleProductChange = (rowId: string, catalogValue: string) => {
    ...
    updateRow(rowId, (current) => {
      ...
      return {
        ...
        productPackageId: catalogItem.productPackageId,
