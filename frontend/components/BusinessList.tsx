...
export const BusinessList: React.FC<BusinessListProps> = ({ businesses = [], onOpenModal }) => {
  ...
  const renderSortIcon = (key: keyof Business) => {
  ...
  const handleDownloadTemplate = () => {
    ...
    const headers = ['Mã lĩnh vực', 'Tên lĩnh vực', 'Mô tả'];
    const sampleRows = [
      ['KD001', 'Phần cứng', 'Thiết bị CNTT'],
      ['KD002', 'Phần mềm', 'Giải pháp số']
    ];
  ...
  const handleExport = (type: 'excel' | 'csv' | 'pdf') => {
    ...
    const headers = ['Mã lĩnh vực', 'Tên lĩnh vực', 'Ngày tạo'];
    const rows = filteredBusinesses.map((row) => [row.domain_code, row.domain_name, row.created_at || '']);
                   ...
                   {[
                     ...
                     <th key={col.key} className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors select-none" onClick={() => handleSort(col.key as keyof Business)}>
                       <div className="flex items-center gap-1">
                         ...
                         {renderSortIcon(col.key as keyof Business)}
                       </div>
                     </th>
                   ))}
