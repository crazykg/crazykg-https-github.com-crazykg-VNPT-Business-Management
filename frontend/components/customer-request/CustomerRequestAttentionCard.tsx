...
const CompactInfoCell: React.FC<{
  ...
  <div className="rounded-xl border border-slate-200 bg-white px-2.5 py-2">
    ...
    <p className={`mt-1 line-clamp-1 text-[12px] font-semibold leading-4 ${valueCls}`}>{value}</p>
    <p className="mt-0.5 line-clamp-1 text-[10px] leading-4 text-slate-400">{hint}</p>
  </div>
);
...
export const CustomerRequestAttentionCard: React.FC<CustomerRequestAttentionCardProps> = ({
  ...
  const shellLayoutCls = isStackedLayout
    ? 'grid gap-2.5'
    : 'grid gap-2.5 lg:grid-cols-[minmax(0,1fr)_290px] lg:items-start';

  return (
    <div
      className={`relative isolate overflow-hidden rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-left shadow-[0_1px_2px_rgba(15,23,42,0.05)] transition ${hoverToneCls} ${className}`}
    >
      <button
        type="button"
      ...
      <div className={`pointer-events-none relative z-0 ${shellLayoutCls}`}>
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] font-semibold text-slate-900">
              {request.ma_yc || request.request_code || '--'}
            </span>
            <span
              className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ${healthMeta.primary.cls}`}
            >
              {healthMeta.primary.label}
            </span>
            {priorityMeta ? (
              <span
                className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ${priorityMeta.cls}`}
              >
          ...
          <p className="mt-2 line-clamp-2 text-[14px] font-semibold leading-5 text-slate-900">
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50/70 px-2.5 py-2.5">
          ...
          <div className="mt-2 flex items-center justify-between gap-3 border-t border-slate-200 pt-2">
            <div className="min-w-0">
              ...
              <p className="mt-1 line-clamp-1 text-[12px] font-semibold leading-4 text-slate-700">
