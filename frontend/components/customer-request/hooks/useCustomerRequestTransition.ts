...
import type { ProjectRaciRow } from '../../../types';
import {
...
const applyFixedDefaultsFromCurrentCase = (
  ...
  const request: Partial<YeuCau> = processDetail?.yeu_cau ?? {};
  ...
  if (!hasValue(payload.received_at)) {
    payload.received_at = normalizeText(legacyRequest.ngay_tiep_nhan ?? request.received_at);
  }
