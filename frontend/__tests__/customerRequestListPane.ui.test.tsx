...
const processOptions: SearchableSelectOption[] = [
...
const buildProps = () => ({
  activeProcessCodes: ['in_progress'] as string[],
  processOptions,
  onProcessCodesChange: vi.fn(),
  requestKeyword: '',
  onRequestKeywordChange: vi.fn(),
  onSubmitKeywordSearch: vi.fn(),
  requestEntityFilter: [] as string[],
  onRequestEntityFilterChange: vi.fn(),
  requestPriorityFilter: [] as string[],
  ...
  tagFilterOptions: [] as SearchableSelectOption[],
  requestTagFilter: [] as string[],
  onRequestTagFilterChange: vi.fn(),
  ...
  requestRoleFilter: '' as const,
  presentation: 'responsive' as const,
});

describe('CustomerRequestListPane UI', () => {
  it('renders desktop table and triggers key filter callbacks', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Khách hàng | Dự án | Sản phẩm' }));
    expect(screen.getByPlaceholderText('Tìm khách hàng, dự án, sản phẩm...')).toBeInTheDocument();
    await user.type(screen.getByPlaceholderText('Tìm khách hàng, dự án, sản phẩm...'), 'VNPT-HCM-21');
    await user.click(screen.getByRole('button', { name: 'VNPT Hồ Chí Minh' }));
    expect(onRequestEntityFilterChange).toHaveBeenCalledWith(['21']);
    ...
    expect(
      screen.getByText((_, element) => element?.textContent === '21 – 40 / 41 bản ghi')
    ).toBeInTheDocument();
  ...
  it('shows mobile card layout and reveals filters when expanded', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Khách hàng | Dự án | Sản phẩm' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Tags' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Ưu tiên' })).toBeInTheDocument();

    await user.click(screen.getByRole('button', { name: 'Tags' }));
    expect(screen.getByPlaceholderText('Tìm tags...')).toBeInTheDocument();
  ...
  it('renders loading and empty table states with correct colspan', () => {
    setViewportWidth(1440);
  ...
  it('hides filter toolbar when showFilterToolbar is false', () => {
    ...
    expect(screen.queryByRole('button', { name: 'Tiến trình' })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Khách hàng | Dự án | Sản phẩm' })).not.toBeInTheDocument();
