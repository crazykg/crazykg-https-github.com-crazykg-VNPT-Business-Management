...
export const ProductList: React.FC<ProductListProps> = ({ products = [], businesses = [], vendors = [], onOpenModal }) => {
  ...
  const filteredProducts = useMemo(() => {
    let result = (products || []).filter(prod => {
      ...
      const matchesStatus = statusFilter 
        ? (statusFilter === 'ACTIVE' ? prod.status === 'Active' : prod.status === 'Inactive')
        : true;
    ...
    if (sortConfig !== null) {
      result.sort((a, b) => {
        ...
        if (sortConfig.key === 'domain') {
            aValue = getDomainName(a.domain);
            bValue = getDomainName(b.domain);
        }
        if (sortConfig.key === 'vendor') {
            aValue = getVendorName(a.vendor);
            bValue = getVendorName(b.vendor);
        }
    ...
    return result;
  }, [products, searchTerm, statusFilter, sortConfig, businesses, vendors]);
  ...
  const handleExport = (type: 'excel' | 'csv' | 'pdf') => {
    ...
    if (type === 'csv') {
      const headers = ['Mã SP', 'Tên SP', 'Lĩnh vực', 'Nhà cung cấp', 'Trạng thái', 'Ngày tạo'];
      const csvContent = [
        headers.join(','),
        ...filteredProducts.map(row => [
          row.id, `"${row.name}"`, `"${getDomainName(row.domain)}"`, `"${getVendorName(row.vendor)}"`, row.status, row.createdDate
        ].join(','))
      ].join('\n');
      const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
      const url = URL.createObjectURL(blob);
               ...
               <thead className="bg-slate-50 border-y border-slate-200">
                 <tr>
                   {[
                     ...
                     { label: 'Lĩnh vực', key: 'domain' },
                     { label: 'Nhà cung cấp', key: 'vendor' },
                 ...
                 {currentData.length > 0 ? (
                   currentData.map((item) => (
                     <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                       ...
                       <td className="px-6 py-4 text-sm text-slate-600">{getDomainName(item.domain)}</td>
                       <td className="px-6 py-4 text-sm text-slate-600">{getVendorName(item.vendor)}</td>
