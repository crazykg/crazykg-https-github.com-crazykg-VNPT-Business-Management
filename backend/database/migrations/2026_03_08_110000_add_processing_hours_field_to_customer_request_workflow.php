...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        $processingStatusId = $this->resolveProcessingStatusId();
        if ($processingStatusId === null) {
            return;
        }

        $rows = DB::table(self::FIELD_TABLE)
            ->where('status_catalog_id', $processingStatusId)
            ->orderBy('sort_order')
            ->orderBy('id')
            ->get()
    ...
    {
        // no-op: this migration only normalizes processing workflow field config.
    }
    ...
    private function resolveProcessingStatusId(): ?int
    {
        ...
        $query = DB::table(self::STATUS_TABLE)
            ->where('status_code', 'DANG_XU_LY');
        if (Schema::hasColumn(self::STATUS_TABLE, 'canonical_status')) {
            $query->orWhere('canonical_status', 'DANG_XU_LY');
        }
