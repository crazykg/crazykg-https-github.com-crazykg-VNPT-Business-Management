...
class CustomerRequestCaseExecutionService
{
    ...
    public function storeWorklog(Request $request, CustomerRequestCase $case, ?int $actorId): JsonResponse
    {
        ...
        $performedByUserId = $this->support->parseNullableInt($request->input('performed_by_user_id')) ?? $actorId;
        ...
        if ($performedByUserId !== null && $this->support->hasColumn('customer_request_cases', 'nguoi_xu_ly_id')) {
            DB::table('customer_request_cases')
                ->where('id', $case->id)
                ->update($this->writeService->filterByTableColumns('customer_request_cases', [
                    'nguoi_xu_ly_id' => $performedByUserId,
                    'updated_by' => $actorId,
                    'updated_at' => now(),
                ]));
            $case->nguoi_xu_ly_id = $performedByUserId;
        }
